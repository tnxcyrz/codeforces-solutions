...
void solve_case(vector<vector<int>> &mults){
  int n, m;
  cin >> n >> m;
  ...
  int cnt = 0;
  vector<int> freqs(m+1);

  for(int i = 0; i < a.size(); i++){
    for(int mult: mults[a[i]]){
      if(mult > m){
        break;
      }
...
int main(){
  ...
  vector<vector<int>> mults(100005);

  for(int i = 1; i < mults.size(); i++){
    for(int j = i; j < mults.size(); j += i){
      mults[j].push_back(i);
    }
  }
