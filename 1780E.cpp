...
void solve_case(){
  ...
  long long ans = max(0LL, R/2LL - L + 1);
  long long a = 1LL;

  while(a < L){
    long long fa = (L + a - 1LL) / a;
    long long x = a, y = L;

    while(x < y){
      long long m = (x + y + 1LL) / 2LL;
      long long fm = (L + m - 1LL) / m;
    ...
    long long b = min(y, R / (fa + 1LL));
    long long add = max(0LL, b - a + 1LL);
    ans += add;
    a = y + 1LL;
  }
